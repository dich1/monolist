class OwnershipsController < ApplicationController
  ...
  def create
    ...
    if params[:type] == 'Have'    # params[:type]の値にHaveボタンが押された時には「Have」
      current_user.have(@item)
    end
    if params[:type] == 'Want'    # Wantボタンがされた時には「Want」が設定されています。
  ...
  def destroy
    ...
    # TODO 紐付けの解除。 
    if params[:type] == 'Have'    # params[:type]の値ににHavedボタンが押された時には「Have」,
      current_user.unhave(@item) 
    end
    if params[:type] == 'Want'    # Wantedボタンがされた時には「Want」が設定されています。
